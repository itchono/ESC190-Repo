...
def optimal_sequnce(treasuremap):
    trace = pathfind_treasure(treasuremap)[2]
...
def pathfind_treasure(treasuremap):
    ...
    def score(x, y):
        if cache[y][x] == -1: # Score is not found yet
            ...
            if (x == 0):
                ...
                trace[y][x] = (0 if score(x, y-1) - 2 > score(x-1, y) - 1 else 1)
    ...
    return score(N-1, M-1), cache, trace
...
if __name__ == "__main__":
    treasuremap = [[2,3,6,1,2],
                   ...
                   [2,1,7,4,3],
                   [9,3,6,2,6]]
    score, cache, trace = maximumScore(treasuremap)
    print(score)
